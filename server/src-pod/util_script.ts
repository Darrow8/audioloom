...
let cleanInstructionsBase = `
You will receive a document that contains an informative article. 
Your job is to remove any table of contents, graphs, images, citations, or other information that does not relate to the text of the article. 
Remove unecessary blank lines, gibberish, or other non-text content as well.
...
export type GPTModel = { version: ChatModel, tokenLimit: number }

export function buildIntructions(article: string, type: InstructionType) : string{
    if(type == InstructionType.CLEAN){
        // clean the article
        return `${cleanInstructionsBase} DOCUMENT_START\n ${article} \nDOCUMENT_END`
    } else if (type == InstructionType.PODCAST){
        // create a podcast script
        return `${podInstructionsBase} DOCUMENT_START\n ${article} \nDOCUMENT_END`
    }
}