...
```bash
generatePod$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
generatePod$ sam local start-api
generatePod$ curl http://localhost:3000/
```
...
```bash
generatePod$ sam logs -n HelloWorldFunction --stack-name generatePod --tail
```
...
```bash
sam delete --stack-name generatePod
```
