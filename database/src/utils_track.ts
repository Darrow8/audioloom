...
import { usefulTrack, Segment } from './utils_sound';
import { AudioFile } from './utils_pod';
...
export async function saveMusicAsAudio(tracks: usefulTrack[], id:string): Promise<AudioFile> {
    ...
    let outputPath = `./content/music/${id}.mp3`; // Replace with your desired local output file name
...
export async function downloadFile(track_id: string, url: string, outputPath: string, segment: Segment) : Promise<number> {
    return new Promise<number>(async (resolve, reject) => {
        ...
        let tempPath = `./content/music/${track_id}_raw.mp3`; // Temporary file path
